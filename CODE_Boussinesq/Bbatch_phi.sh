...
# Run parameters
# The following three are boolean variables "1" True "0" False
...
# Options: "Boussinesq"=0  
declare -r prob=0 

# Declare which function to approximate 
#  DNN_whichfun = 0 -> "_u_"; 
#  DNN_whichfun = 1 -> "_p_"; 
#  DNN_whichfun = 2 -> "_phi_"; 
declare -r DNN_whichfun=2 
...
# WARNING 
# MAKE SURE THE NUMBER IN  DNN_nb_trainingpts IS LESS OR EQUAL TO THE NUMBER OF TRAINING POINTS (LINE 151 BELOW)
if [ "$prob" = 0 ]; then
    declare -r problem="Boussinesq"
    # DNN PARAMETERS
...
do
    ...
    do
        ...
        if [ "$prob" = 0 ]; then
            declare  train_pts=500 # ~0.8G each set
            if [ "$input_dim" -eq "4" ]; then
                declare  SG_level=5  #1105  points 0.16Gx3 (x3 for dimensions)
            elif [ "$input_dim" -eq "8" ]; then
                declare  SG_level=3  #3937  points 0.6Gx3  (x3 for dimensions)
            else
                echo "input dim is 4,8"
            fi
        else
            echo "Problem must be Boussinesq"
        fi

        ############## WARNING ! ##############################
        for trialss in {1..8..1} # this is only useful if you run the code on a local machine. 
        do
            if [ "$UseFEM_tn" = 1 ]; then
                ## Args: FEM
                # 1 =  $run_ID
